...
```shell
Pwn1Sec@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare
...
```shell
Pwn1Sec@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare -user test -password test
...
```shell
Pwn1Sec@htb[/htb]$ sudo pip3 install pyftpdlib
```
...
```shell
Pwn1Sec@htb[/htb]$ sudo python3 -m pyftpdlib --port 21
...
```shell
Pwn1Sec@htb[/htb]$ echo  | base64 -d > hosts
```

  Decode Base64 String in Linux

```shell
Pwn1Sec@htb[/htb]$ md5sum hosts 
...
```shell
Pwn1Sec@htb[/htb]$ pip3 install uploadserver
...
```shell
Pwn1Sec@htb[/htb]$ echo <base64> | base64 -d -w 0 > hosts
```
...
```shell
Pwn1Sec@htb[/htb]$ sudo pip install wsgidav cheroot
...
```shell
Pwn1Sec@htb[/htb]$ sudo wsgidav --host=0.0.0.0 --port=80 --root=/tmp --auth=anonymous 
...
```shell
Pwn1Sec@htb[/htb]$ sudo python3 -m pyftpdlib --port 21 --write
