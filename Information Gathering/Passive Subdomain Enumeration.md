...
Pwn1Sec@htb[/htb]$ curl -s "https://crt.sh/?q=${TARGET}&output=json" | jq -r '.[] | "\(.name_value)\n\(.common_name)"' | sort -u > "${TARGET}_crt.sh.txt"
```

```shell
Pwn1Sec@htb[/htb]$ head -n20 facebook.com_crt.sh.txt
...
```shell
Pwn1Sec@htb[/htb]$ export TARGET="facebook.com"
Pwn1Sec@htb[/htb]$ export PORT="443"
Pwn1Sec@htb[/htb]$ openssl s_client -ign_eof 2>/dev/null <<<$'HEAD / HTTP/1.0\r\n\r' -connect "${TARGET}:${PORT}" | openssl x509 -noout -text -in - | grep 'DNS' | sed -e 's|DNS:|\n|g' -e 's|^\*.*||g' | tr -d ',' | sort -u
...
```shell
Pwn1Sec@htb[/htb]$ cat sources.txt
...
```shell
Pwn1Sec@htb[/htb]$ export TARGET="facebook.com"
Pwn1Sec@htb[/htb]$ cat sources.txt | while read source; do theHarvester -d "${TARGET}" -b $source -f "${source}_${TARGET}";done
...
```shell
Pwn1Sec@htb[/htb]$ cat *.json | jq -r '.hosts[]' 2>/dev/null | cut -d':' -f 1 | sort -u > "${TARGET}_theHarvester.txt"
```

Now we can merge all the passive reconnaissance files via: 

```shell
Pwn1Sec@htb[/htb]$ cat facebook.com_*.txt | sort -u > facebook.com_subdomains_passive.txt
Pwn1Sec@htb[/htb]$ cat facebook.com_subdomains_passive.txt | wc -l
