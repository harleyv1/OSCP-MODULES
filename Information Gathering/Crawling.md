...
Pwn1Sec@htb[/htb]$ ffuf -recursion -recursion-depth 1 -u http://192.168.10.10/FUZZ -w /opt/useful/SecLists/Discovery/Web-Content/raft-small-directories-lowercase.txt
...
```shell
Pwn1Sec@htb[/htb]$ cewl -m5 --lowercase -w wordlist.txt http://192.168.10.10
```
...
```shell
Pwn1Sec@htb[/htb]$ ffuf -w ./folders.txt:FOLDERS,./wordlist.txt:WORDLIST,./extensions.txt:EXTENSIONS -u http://192.168.10.10/FOLDERS/WORDLISTEXTENSIONS
...
```shell-session
Pwn1Sec@htb[/htb]$ curl http://192.168.10.10/wp-content/secret~
